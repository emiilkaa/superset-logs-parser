...
from repository.log_repository import get_popular_actions


def open_popular_actions_window(master):
    logs = get_popular_actions()
    ...
    addButton(toolbar, 'Save results', partial(save_dataframe, logs,
                                               f'actions_by_dashboard_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.csv',
                                               root), side='right')
